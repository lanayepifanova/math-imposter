    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/20 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 relative animate-in zoom-in-95 duration-300 border border-stone-100">
        ...
        <div className="text-center mb-6">
          <h3 className="text-2xl font-playfair font-bold text-stone-800">New Category</h3>
          <div className="h-1 w-12 bg-primary mx-auto rounded-full mt-2 opacity-50"></div>
        </div>

        <div className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-stone-600 uppercase tracking-wider">Category Name</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g. Movies, Animals"
              className="w-full p-4 rounded-xl bg-stone-50 border border-stone-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-lato"
            />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-bold text-stone-600 uppercase tracking-wider">
              Words <span className="text-stone-400 font-normal normal-case">(comma separated)</span>
            </label>
            <textarea
              value={wordsInput}
              onChange={(e) => setWordsInput(e.target.value)}
              placeholder="Lion, Tiger, Bear, Elephant, Giraffe..."
              className="w-full p-4 rounded-xl bg-stone-50 border border-stone-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-lato min-h-[120px] resize-none"
            />
          </div>
